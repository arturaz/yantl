...
object ValidatorRule {
  ...
  case class SmallerThan[+A](minimum: A, actual: A) derives CanEqual {
    override def toString(): String =
      s"Cannot be smaller than $minimum, was $actual."
  }

  /** The value must not be greater than the maximum, but it was. */
  case class LargerThan[+A](maximum: A, actual: A) derives CanEqual {
    override def toString(): String =
      s"Cannot be larger than $maximum, was $actual."
  }

  type OutOfRange[+A] = SmallerThan[A] | LargerThan[A]

  /** The value must not be shorter than the minimum length, but it was. */
  case class UnderMinLength[+A](minLength: Int, actualLength: Int, actual: A)
      derives CanEqual {
    override def toString(): String =
      s"Cannot be shorter than $minLength ${English.characters(minLength)}, " +
        s"was $actualLength ${English.characters(actualLength)}."
  }

  /** The value must not be longer than the maximum length, but it was. */
  case class MaxLengthExceeded[+A](maxLength: Int, actualLength: Int, actual: A)
      derives CanEqual {
    override def toString(): String =
  ...
  def minValue[A](minimum: A)(using
      num: Numeric[A]
  ): ValidatorRule[A, SmallerThan[A]] =
    a => if (num.gt(a, minimum)) None else Some(SmallerThan(minimum, a))

  /** Requires the value to be less than or equal to the maximum. */
  def maxValue[A](maximum: A)(using
      num: Numeric[A]
  ): ValidatorRule[A, LargerThan[A]] =
    a => if (num.lt(a, maximum)) None else Some(LargerThan(maximum, a))

  /** Requires the value to be between the minimum and maximum. */
  def between[A](minimum: A, maximum: A)(using
      num: Numeric[A]
  ): ValidatorRule[A, SmallerThan[A] | LargerThan[A]] =
    a =>
      if (num.lt(a, minimum)) Some(SmallerThan(minimum, a))
      else if (num.gt(a, maximum)) Some(LargerThan(maximum, a))
      else None

  /** Requires the value to not be shorter than the minimum length. */
  def minLength[A](
      minLength: Int,
      getLength: A => Int
  ): ValidatorRule[A, UnderMinLength[A]] =
    a => {
      val length = getLength(a)

      if (length < minLength) Some(UnderMinLength(minLength, length, a))
      else None
    }

  /** Requires the value to not be shorter than the minimum length. */
  def minLength(minLength: Int): ValidatorRule[String, UnderMinLength[String]] =
    this.minLength(minLength, _.length)

  /** Requires the value to not be longer than the maximum length. */
  def maxLength[A](
      maxLength: Int,
      getLength: A => Int
  ): ValidatorRule[A, MaxLengthExceeded[A]] =
    a => {
      val length = getLength(a)
  ...
  def maxLength(
      maxLength: Int
  ): ValidatorRule[String, MaxLengthExceeded[String]] =
  ...
  def nonBlankString: ValidatorRule[String, WasBlank] =
    a => if (a.trim().isEmpty()) Some(WasBlank(a)) else None

  /** Requires the string to not have leading or trailing whitespace. */
  def withoutSurroundingWhitespace
      : ValidatorRule[String, HadSurroundingWhitespace] =
